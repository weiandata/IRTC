...
{\LightFont\fontsize{11.5}{18}\selectfont\color{IRTCgoldsoft}%
 代码量 · 文字量 · 技术路径 · 实现效果 \quad|\quad 版本 1.1.2，已发布于 CRAN}\par
...
{\LightFont\fontsize{10.5}{16}\selectfont\color{white}%
 软件版本 1.1.2（2026-08-27）\quad · \quad 本说明 2026 年 8 月\par}
\vspace{0.7cm}
...
以仓库当前（版本 1.1.2）内容统计，项目共 \textbf{255 个源文件、21{,}380 行}。
其中真正的实现代码（R 与 C++）约 1.24 万行，自动化测试 5{,}699 行，
...
\midrule
\strut R 源码 \IRTColdtexttt{R/} & 133 & 11{,}144 &
估计层、数据层、统计层、输出层、AI 接口与结构化报错 \\
\strut C++ 源码 \IRTColdtexttt{src/} & 13 & 1{,}282 &
并行 E 步、概率与充分统计量、2PL/GPCM 求解、流式引擎 \\
\strut 自动化测试 \IRTColdtexttt{tests/} & 64 & 5{,}699 &
340 个测试块、1{,}298 处断言 \\
\strut 帮助文档 \IRTColdtexttt{man/} & 22 & 1{,}345 &
每个导出函数的 R 标准帮助页与示例 \\
\strut 机器接口 \IRTColdtexttt{inst/} & 2 & 199 &
\IRTColdtexttt{llms.txt} API 摘要、版权声明 \\
\strut 构建与排版 \IRTColdtexttt{scripts/} & 19 & 1{,}625 &
发布校验、基准测试、手册 PDF 设计与生成 \\
\strut 示例 \IRTColdtexttt{examples/} & 2 & 86 &
端到端使用示例 \\
\midrule
\strut\textbf{合计} & \textbf{255} & \textbf{21{,}380} & \\
\bottomrule
\end{tabular}\par}

\vspace{10pt}
\textbf{去掉空行与纯注释后的有效代码行}：R 层 8{,}978 行、C++ 层 964 行、
测试 4{,}805 行、帮助文档 1{,}243 行。

\vspace{4pt}
\textbf{对外接口规模}：21 个导出函数 + 13 个 S3 方法（\IRTColdtexttt{summary}、
...
\vspace{4pt}
中文手册的 PDF 排版工程（封面、引言、目录、章节页、尾页与配色体系）共 6 个设计
文件，已计入 \IRTColdtexttt{scripts/} 的行数中。1.1.2 另增一份《缺陷处置记录》，
逐条回应一次真实生产使用中提出的技术记录。
...
\midrule
\strut 发布状态 &
已通过 CRAN 审核并上架，使用者一条
\IRTColdtexttt{install.packages("IRTC")} 即可安装；Windows 与 macOS 直接获得
编译好的二进制包 \\
\strut 质量验证 &
\IRTColdtexttt{R CMD check -{}-as-cran} 0 错误 0 警告 0 提示（含 PDF 手册构建）；
1{,}298 项自动化测试全部通过；整体代码覆盖率 96.0\%，关键模块 ≥ 95\%；
macOS / Windows / Linux 三平台持续集成 \\
\strut 生产验证 &
1.1.2 经一次 85{,}035 人的全国性调查实际使用检验，并用其原始 SPSS 文件
（85{,}281 行 × 243 列）做全链路复核：按答案键计分与原系统 85 万个计分单元
逐格一致，估计结果与独立计算的基线逐位吻合 \\
\strut 规模与性能 &
...
四件事一并做完，并用 CRAN 的公开审核、96\% 的测试覆盖率、百万级规模验证，
以及一次 8.5 万人真实调查的全链路复核，证明它能用、可信、跑得动。
