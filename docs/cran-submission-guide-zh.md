...
# 3.2 构建源码包
R CMD build IRTC
#   -> 生成 IRTC_0.1.0.tar.gz

# 3.3 以 CRAN 标准检查
R CMD check IRTC_0.1.0.tar.gz --as-cran
```
...
4. **结果**：
   ...
   - **要求修改** → 按邮件意见改，**提升版本号**（如 0.1.0 → 0.1.1），重跑检查，重新提交，并在 `cran-comments.md` 里写 “Resubmission” 及本次改了什么。
