...
## File Name: irtc_mml_2pl_mstep_slope.R

irtc_mml_2pl_mstep_slope <- function (B_orig, B, B_obs, B.fixed, max.increment, nitems, A,
            ...
            maxcat=NULL, use_rcpp=FALSE, use_rcpp_calc_prob=TRUE, n_threads=1L )
{
    ...
    while (!converge & Biter <= Msteps) {
        probability_result <- irtc_mml_calc_prob(
            iIndex=items.temp, A=A, AXsi=AXsi, B=B, xsi=xsi,
        ...
        for (dd in 1L:ndim) {
            ...
            if (use_rcpp) {
                sufficient_statistics <- irtc_rcpp_mml_2pl_mstep_item_slopes_suffstat(
                    rprobs=as.vector(rprobs), items_temp=items.temp,
                    theta=theta, dd=dd-1, LIT=LIT, TP=TP,
                    nitems=nitems, maxcat=maxcat, maxK=maxK,
                    itemwt=itemwt, xxf_=xxf, xbar_=xbar,
                    xbar2_=xbar2, irtmodel=irtmodel, xtemp_=xtemp,
                    items_conv=items.conv1, n_threads=n_threads
                )
            } else {
                sufficient_statistics <- irtc_mml_2pl_mstep_item_slopes_suffstat_R(
                    rprobs=rprobs, maxK=maxK, LIT=LIT, TP=TP,
                    itemwt=itemwt, theta=theta, dd=dd,
                    items.temp=items.temp, items.conv=items.conv,
                    xbar=xbar, xbar2=xbar2, xxf=xxf,
                    xtemp=xtemp, irtmodel=irtmodel
                )
            }
            xbar <- sufficient_statistics$xbar
            xbar2 <- sufficient_statistics$xbar2
            xxf <- sufficient_statistics$xxf
            xtemp <- sufficient_statistics$xtemp

            if (irtmodel %in% c("GPCM", "GPCM.design", "GPCM.groups")) {
                B_obs.temp <- B_obs
                category_scores <- irtc_matrix2(mK - 1, LIT, maxK)
                B_obs.temp[items.temp, , dd] <- (
                    category_scores * B_obs[items.temp, , dd]
                )
                xbar[is.na(xbar)] <- 0
                xbar.temp <- category_scores * xbar
            ...
            if (irtmodel %in% c("GPCM", "GPCM.groups")) {
                ...
                if (irtmodel == "GPCM.groups") {
                    diff.temp <- irtc_aggregate_derivative_information(
                        deriv=diff.temp, groups=est.slopegroups
                    )
                    deriv.temp <- irtc_aggregate_derivative_information(
                        deriv=deriv.temp, groups=est.slopegroups
                    )
                }

                increment.temp <- diff.temp * abs(1 / (deriv.temp + eps))
                increment.temp <- irtc_trim_increment(
                ...
                increment <- irtc_outer( increment.temp, mK - 1)
                if (Biter == 1) {
                    se.B[, , dd] <- irtc_outer(
                        sqrt(1 / abs(deriv.temp)), mK - 1
                    )
                }
            }
            ...
            if (irtmodel == "GPCM.design") {
                ...
                increment <- irtc_outer( increment.temp, mK-1)
                d1 <- irtc_outer(1 / abs(deriv.temp), mK - 1)
                LL <- ncol(d1)
                for (ll in 1L:LL) {
                    m1 <- sqrt(diag(
                        E %*% d1[, ll] %*% t(d1[, ll]) %*% t(E)
            ...
            if (irtmodel == "2PL.groups") {
                a1 <- stats::aggregate(
                    B_obs[, , dd] - xbar, list(est.slopegroups), sum
                )
                a2 <- stats::aggregate(
                    xbar2 - xxf, list(est.slopegroups), sum
                )
                ...
                if (Biter == 1) {
                    se.B[, , dd] <- sqrt(1 / abs(deriv.temp[ind.match, ]))
                }
            }
    ...
    if (B_acceleration$acceleration != "none") {
        B_acceleration <- irtc_accelerate_parameters(
            xsi_acceleration=B_acceleration, xsi=as.vector(B),
            iter=iter, itermin=3
        )
    ...
    list(
        "B"=B, "basispar"=basispar, "se.B"=se.B, Biter=Biter,
        B_acceleration=B_acceleration, B_change=B_change,
        increments_msteps=increments_msteps
    )
}

Mstep_slope.v2 <- irtc_mml_2pl_mstep_slope
